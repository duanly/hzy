...
import { autoSort, orderCol, orderCols, sortCol, affinity, revealCols, minXiOf } from './sort.ts';
...
function SeatBox({ seat, mySeat, rel, label, g, isTurn, dealer, onAvatar, onKick, onInvite, bubbles, horizontal, ringFrac, side, huTag, holdCard, holdCid, landCard, landDone, landMeldCard, landMeldSize, landMeldAgain, holdMeldCard, holdMeldsFrom, holdCount, huMark, huGroups, huGroupIdx, fouls }: { seat: RoomView['seats'][number]; mySeat?: number; landCard?: Kind | null; landDone?: boolean; landMeldCard?: Kind | null; landMeldSize?: number; landMeldAgain?: boolean; huGroupIdx?: number; label: string; g: any; fouls?: number[]; rel: number; isTurn: boolean; dealer: boolean; onAvatar: (id: number) => void; onKick?: (seat: number) => void; onInvite?: (seat: number) => void; huMark?: Kind | null; huGroups?: Kind[][] | null; bubbles: { id: number; seat: number; text: string; ms?: number }[]; horizontal?: boolean; ringFrac?: number; side?: 'left' | 'right'; huTag?: string; holdCard?: Kind | null; holdCid?: number; holdMeldCard?: Kind | null; holdMeldsFrom?: number; holdCount?: number }) {
        ...
        {p && <div className={`seat-body ${side === 'right' ? 'rev' : ''}`}>
          <div className="seat-cards">
            ...
            {(() => {
              ...
              const inMeld = inHand >= 0 || huCol ? -1 : markIn(shown.map(m => m.cards as Kind[]), huMark);
...
export function HuPanel({ hu, names, round }: { hu: { seat: number; card: number; ziMo: boolean; fromSeat: number; detail: any }; names: string[]; round?: number }) {
  ...
  return <>
    ...
    {(() => {
      ...
      const mMark = hMark >= 0 ? -1 : markIn((d.melds as Meld[]).map(m => m.cards as Kind[]), want);
      ...
      <div key={i} className="reveal-row">
        ...
        <div className="row" style={{ flexWrap: 'wrap', gap: 4 }}>
          ...
          {(() => {
            ...
            const inMeld = inHu || inHand >= 0 ? -1 : markIn(ms.map(m => m.cards as Kind[]), want);
